...
export default function InsightsAccess({ onAuthenticated }: Props) {
  ...
  const [pin, setPin] = useState(""); const [state, setState] = useState<AccessState>("checking"); const [message, setMessage] = useState(""); const [lockedUntil, setLockedUntil] = useState<number | null>(null); const [now, setNow] = useState(() => Date.now());
  const checkSession = async () => {
    try { const response = await fetch("/.netlify/functions/admin-session", { credentials: "same-origin" }); const body = await response.json(); if (body.authenticated) onAuthenticated(); else if (!body.configured) setState("unconfigured"); else setState("ready"); } catch { setState("unconfigured"); }
  };
  useEffect(() => { void checkSession(); }, []);
  // Focus once the input is enabled again; focusing a disabled input is ignored and dismisses mobile keyboards.
  useEffect(() => { if (state === "ready") inputRef.current?.focus(); }, [state]);
  useEffect(() => { if (!lockedUntil) return; const timer = window.setInterval(() => { const current = Date.now(); setNow(current); if (lockedUntil <= current) { setLockedUntil(null); setState("ready"); setMessage(""); } }, 1000); return () => window.clearInterval(timer); }, [lockedUntil]);
  const submit = async (candidate: string) => {
    if (candidate.length !== 6 || state === "verifying") return;
    setState("verifying"); setMessage("");
    try { const response = await fetch("/.netlify/functions/admin-login", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ pin: candidate }) }); const body = await response.json(); if (response.ok && body.ok) { onAuthenticated(); return; } if (body.error === "locked" && body.lockedUntil) { setNow(Date.now()); setLockedUntil(new Date(body.lockedUntil).getTime()); setState("locked"); setPin(""); return; } setPin(""); setState("ready"); setMessage(body.error === "incorrect_pin" ? `Incorrect access code. ${body.attemptsRemaining} ${body.attemptsRemaining === 1 ? "attempt" : "attempts"} remaining.` : "Unable to verify access right now."); } catch { setPin(""); setState("ready"); setMessage("Unable to verify access right now."); }
  };
