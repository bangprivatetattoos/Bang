...
async function dashboard(client, filters, page, pageSize) {
  ...
  const unique = new Set(sessions.map((session) => session.visitor_id)).size; const starts = new Set(activeEvents.filter((event) => event.event_name === "booking_start").map((event) => event.session_id)).size; const submissions = new Set(activeBookings.map((booking) => booking.analytics_session_id)).size || new Set(activeEvents.filter((event) => event.event_name === "booking_success").map((event) => event.session_id)).size; const handoffs = new Set(activeEvents.filter((event) => event.event_name === "whatsapp_click").map((event) => event.session_id)).size; const home = new Set(activeEvents.filter((event) => event.event_name === "home_call_click" || event.event_name === "home_call_view").map((event) => event.session_id)).size;
  ...
  const portfolio = activeEvents.filter((event) => event.event_name === "portfolio_image_open").map((event, index) => ({ image: event.entity_id ?? `Work image ${index + 1}`, artist: event.metadata?.artist_name ?? "Unassigned", views: 1, clicks: 1, artistVisits: 0, bookings: activeBookings.some((booking) => booking.analytics_session_id === event.session_id) ? 1 : 0 }));
  ...
  const bucketLabel = (date) => multiDay ? date.toISOString().slice(5, 10) : `${String(date.getHours()).padStart(2, "0")}:00`;
  for (const session of sessions) {
    const label = bucketLabel(new Date(session.started_at));
    const item = trafficBuckets.get(label) ?? { label, sessions: 0, visitorIds: new Set() };
    item.sessions += 1;
    item.visitorIds.add(session.visitor_id);
    trafficBuckets.set(label, item);
  }
  const traffic = [...trafficBuckets.values()].map((item) => ({
    label: item.label,
    ...
    bookings: activeBookings.filter((booking) => {
      ...
      return bookingSession && bucketLabel(new Date(bookingSession.started_at)) === item.label;
    }).length,
  }));
  const content = count(activeEvents.filter((event) => event.event_name === "page_view"), (event) => event.page_path).map(([pagePath, views]) => ({ page: pagePath, views, engagement: "—", starts: new Set(activeEvents.filter((event) => event.event_name === "booking_start" && event.page_path === pagePath).map((event) => event.session_id)).size, exitRate: 0 })).sort((a, b) => b.views - a.views);
...
export default async (request) => {
  ...
  const url = new URL(request.url); const range = url.searchParams.get("range") ?? "today"; const page = Math.max(1, Number(url.searchParams.get("page") ?? 1)); const pageSizeRaw = Number(url.searchParams.get("pageSize") ?? 25); const pageSize = PAGE_SIZES.has(pageSizeRaw) ? pageSizeRaw : 25;
